...
from .utils.manage_tool import Tle
...
class TLE(commands.Cog):
    ...
    @app_commands.command(name='tle', description='어떤 TLE 영상을 보고 싶으신가요? 주간 경쟁, 엘리트컵부터 다른 TLE까지 알려드립니다!')
    @app_commands.describe(tle_type = '어떤 TLE 타입인가요?', area = '맵을 고르세요!', car_name ='어떤 차량을 찾아보시겠어요?')
    @app_commands.rename(tle_type = 'TLE 타입',area = '맵', car_name = '차량')
    @app_commands.guild_only()
    async def TLEs(self, interaction: discord.Interaction, tle_type : str, area : str, car_name : str):

        # ./utils/manage_tool.py 참고
        tle_data = await Tle.type_of_tle()
        map_data = await Tle.Area_db()
        car_data = await Tle.CarName_db()
        
        lap_time_data = await Tle.LapTime_db()
        link_data = await Tle.Link_db()
        
        tle_arr = numpy.array(tle_data)
        car_arr = numpy.array(car_data)
            
        a = numpy.where(tle_arr == area)
        ...
        try:
            ...
            # 정상 실행
            if same2 and (area in set(map_data)):
    ...
    async def tle_type_autocompletion(
        ...
        # 차량 리스트 선언
        tle_type_data = await Tle.type_of_tle()
    ...
    async def area_autocompletion(
        ...
        rest_list = list(filter(lambda x: map_data[x] == str(aa[0]), range(len(tle_type_data))))
    ...
    async def car_autocompletion(
        ...
        aa = list(interaction.namespace.__dict__.values())
        rest_list_1 = list(filter(lambda x: map_data[x] == str(aa[0]), range(len(tle_type_data))))
