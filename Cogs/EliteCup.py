...
class ElitecupKR(commands.Cog):
    def __init__(self, app : commands.Bot):
        self.app = app
    
    
    @app_commands.command(name='엘리트', description='주간 경쟁, 엘리트컵부터 다른 TLE까지 알려드립니다!')
    @app_commands.describe(class_type = '어떤 클래스인가요?', area = '맵을 고르세요!', car_name ='어떤 차량을 찾아보시겠어요?')
    @app_commands.rename(class_type = '타입', area = '맵', car_name = '차량')
    @app_commands.guild_only()
    async def elite(self, interaction: discord.Interaction, class_type : str, area : str, car_name : str):
        ...
        class_data = await EC.Class_db()
        map_data = await EC.Area_db()
        car_data = await EC.CarName_db()
        lap_time_data = await EC.LapTime_db()
        link_data = await EC.Link_db()
        ...
        embed1.add_field(name= '검색', value= f'{class_type} / {area} / {car_name}')
        ...
        log_embed_error.add_field(name='리스트에 없는 값 입력' , value= f'{class_type} / {area} / {car_name}', inline= False)
        ...
        try:
            ...
            # 정상 실행
            if same_num_list and (class_type in set(class_data)):
                ...
                log_embed = discord.Embed(title= '정상 실행', description= f'엘리트', colour= etc)
    ...
    @elite.autocomplete(name= 'class_type')
    async def class_type_autocompletion(
        self,
    ...
    async def area_autocompletion(
        ...
        class_type_data.pop(0); map_data.pop(0)
    ...
    async def car_autocompletion(
        ...
        class_type_data.pop(0); map_data.pop(0); car_data.pop(0)
        # class_autocompletion의 결과와 연동이 어려워 같은 방법 반복
        aa = list(interaction.namespace.__dict__.values())
        rest_list_1 = list(filter(lambda x: class_type_data[x] == str(aa[0]), range(len(class_type_data))))
...
async def setup(app : commands.Bot):
    await app.add_cog(ElitecupKR(app))