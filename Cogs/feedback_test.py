...
class FixModal(Modal, title = '데이터 수정 요청'):    
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        confirm = f"정상 실행 > {await print_time.get_UTC()} > feedback > 서버: {interaction.guild.name} > 채널 : {interaction.channel.name} > 실행자: {interaction.user.display_name} > Req Fix"
...
class SuggestModal(Modal, title= '기타 제안'):    
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        confirm = f"정상 실행 > {await print_time.get_UTC()} > feedback > 서버: {interaction.guild.name} > 채널 : {interaction.channel.name} > 실행자: {interaction.user.display_name} > Suggest"
        await log_ch.send(confirm); print(confirm)  
...
class ReportModal(Modal, title = '봇 작동 신고'):    
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        confirm = f"정상 실행 > {await print_time.get_UTC()} > feedback > 서버: {interaction.guild.name} > 채널 : {interaction.channel.name} > 실행자: {interaction.user.display_name} > Report"
        await log_ch.send(confirm); print(confirm)  
