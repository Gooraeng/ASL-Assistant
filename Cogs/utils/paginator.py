...
class goto_modal(discord.ui.Modal, title="페이지 이동"):
    def __init__(self, button):
        super().__init__()
        self.button = button
        self.page_num = discord.ui.TextInput(
            label='페이지',
            placeholder=f'페이지 번호 : 1-{len(self.button.view.embeds)}',
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if num in range(len(view.embeds)):
                ...
                return await interaction.followup.send(content="잘못된 번호 - 종료 중", ephemeral=True)
            ...
            return await interaction.response.send_message(content="번호를 입력해주세요", ephemeral=True)
