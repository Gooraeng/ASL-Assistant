...
class spec(commands.Cog):
    ...
    async def car(self, interaction : discord.Interaction, car_name : str):
        ...
        embed.add_field(name='조회 불가능 차량', value= get_check_list_, inline= False)
        ...
        # 파일이 존재하지 않음
            ...
            if FileNotFoundError:
                if car_name in get_check_list:
                    embed1 = discord.Embed(title='오류', description=f'< {car_name} >의 정보가 현재 없습니다. 조회 불가능한 차량 리스트를 보고 다시 시도해주세요!', colour= 0xff0000)
