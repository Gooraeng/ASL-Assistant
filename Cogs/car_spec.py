...
class spec(commands.Cog):
    ...
    async def car(self, interaction : discord.Interaction, car_name : str):
        ...
        if get_check_list == None:
            ...
            get_check_list_ = (' / ').join(s for s in get_check_list)
        
        
        embed1 = discord.Embed(title='주의', description=f'정보가 누락되거나 정확하지 않을 수 있습니다. 문제 발견 시 ASL Assistant 디스코드 서버를 통해 신고해주십시오! (/link 입력)', colour=0x7fe6e4)
        embed1.add_field(name='**<경고>**',value='All list From "MEI Car list", All images from "A9-Database". Type "Ref" For details. ', inline=False)
        embed1.add_field(name='',value='')
        embed1.add_field(name='조회 불가능 차량', value= get_check_list_, inline= False)
        ...
        # 파일이 존재하지 않음
            ...
            if FileNotFoundError:
                if car_name in get_check_list:
                    ...
                    embed2.add_field(name='조회 불가능 차량', value= get_check_list_, inline= False)
                ...
                embed4 = discord.Embed(title='어이쿠!', description=f'지금은 조회할 수 없습니다! 잠시 후에 다시 시도해주세요.',colour=0xff0000)
